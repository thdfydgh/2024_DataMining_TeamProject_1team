...
def f1_read():
    ...
    ## 2022년도 csv 파일들을 DataFrame으로 불러와서 concat
    path = './교통사고_2020/'
    ...
    ## 2023년도 csv 파일들을 DataFrame으로 불러와서 concat
    path = './교통사고_2021/'
    file_list = os.listdir(path)
...
def f7_divide_dataset_for_oneSVM(df):
    ...
    X_neg_train, X_neg_test, y_neg_train,y_neg_test= train_test_split(X_neg,y_neg,test_size=test_size)

    #양성데이터만 존재하는 데이터프레임
    pos_df= df[df['사망사고여부']==1]
    pos_df=pos_df.sample(frac=test_size)
    y_pos= pos_df['사망사고여부']
